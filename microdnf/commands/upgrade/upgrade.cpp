...
class TransCB : public libdnf::rpm::TransactionCB {
    void install_start(const libdnf::rpm::Transaction::Item * /*item*/, const std::string & name, uint64_t total) override {
        std::cout << "Start: " << name << "Size: " << total << std::endl;
    }
    void install_progress(const libdnf::rpm::Transaction::Item * /*item*/, const std::string & name, uint64_t amount, uint64_t total) override {
        std::cout << "Progress: " << name << " " << amount << '/' << total << std::endl;
    }
};
...
void CmdUpgrade::run(Context & ctx) {
    ...
    std::vector<std::unique_ptr<libdnf::rpm::Transaction::Item>> transaction_items;
    auto ts = libdnf::rpm::Transaction(ctx.base);
    ts.register_cb(std::make_unique<TransCB>());
    for (auto package : result_pset) {
